...
def mock_api_base():
    api = MagicMock()
    api.get_me = AsyncMock(return_value={"id": "bot_id", "username": "goose-bot"})
    api.get_direct_channels = AsyncMock(return_value=[])
    api.get_my_teams = AsyncMock(return_value=[{"id": "team_1"}])
    api.get_my_channels = AsyncMock(return_value=[{"id": "chan_1", "type": "O"}])
...
async def wait_for_prompts(prompts_list, count, timeout=3.0):
    ...
    while len(prompts_list) < count:
        if asyncio.get_event_loop().time() - start > timeout:
            raise TimeoutError(f"Timed out waiting for {count} prompts. Got {len(prompts_list)}. Prompts: {prompts_list}")
        await asyncio.sleep(0.05)

@pytest.mark.asyncio
async def test_bridge_integration_flow_with_goose_process(base_config, mock_api_base):
    # Setup stateful get_channel_posts
    test_post = {
        "id": "post_123", "user_id": "user_1", "channel_id": "chan_1",
        "message": "@goose-bot What is the meaning of life?", "create_at": 1000
    }
    ...
    async def side_effect_get_posts(channel_id, since):
        ...
        return {"posts": {}, "order": []}
    mock_api_base.get_channel_posts = AsyncMock(side_effect=side_effect_get_posts)
    ...
    bridge = MattermostBridge(api=mock_api_base, config=base_config, goose_client_factory=lambda u: GooseACPClient(u, config=base_config))
    bridge.last_since = 0
...
async def test_bridge_multi_turn_catchup_flow(base_config, mock_api_base):
    ...
    bridge = MattermostBridge(api=mock_api_base, config=base_config, goose_client_factory=goose_factory)
    bridge.last_since = 0
    ...
    with patch('mattermost_bridge.load_user_mapping', return_value={"user_1": "linux_alice"}), \
        ...
        async def complex_simulator():
            ...
            for i in range(2, 30):
                ...
                if i == 2: # session/new for turn 1
                    mock_process.feed_stdout({"jsonrpc": "2.0", "id": i, "result": {"sessionId": "s1"}})
                else: # Success response for prompts and session/use
                    mock_process.feed_stdout({"jsonrpc": "2.0", "id": i, "result": {"status": "completed"}})
        ...
        try:
            ...
            mock_api_base.get_thread = AsyncMock(return_value={"posts": {"p1": {"id": "p1", "user_id": "user_1", "message": "@goose-bot hi"}}})
            ...
            await posts_queue.put([
                ...
                {"id": "p3", "user_id": "user_1", "channel_id": "chan_1", "root_id": "p1", "message": "@goose-bot check", "create_at": 1200}
            ])
            await wait_for_prompts(prompts_received, 2)

            # Turn 3: Clear (using p1 as root)
            await posts_queue.put([{"id": "p4", "user_id": "user_1", "channel_id": "chan_1", "root_id": "p1", "message": "@goose-bot next", "create_at": 1300}])
...
@pytest.mark.asyncio
async def test_bridge_integration_ignore_empty_messages(base_config, mock_api_base):
    posts_queue = asyncio.Queue()
    async def side_effect_get_posts(channel_id, since):
        try:
            batch = posts_queue.get_nowait()
    ...
    with patch('mattermost_bridge.load_user_mapping', return_value={"user_1": "linux_alice"}), \
        ...
        try:
            ...
            mock_api_base.get_thread = AsyncMock(return_value={
                "posts": {
                    "p1": {"id": "p1", "user_id": "user_1", "message": "hello"},
                    "p2": {"id": "p2", "user_id": "user_1", "message": "  "},
                    "p3": {"id": "p3", "user_id": "user_1", "message": "@goose-bot hi"}
                }
            })
            await posts_queue.put([{"id": "p3", "user_id": "user_1", "channel_id": "chan_1", "root_id": "p1", "message": "@goose-bot hi", "create_at": 1000}])
            await wait_for_prompts(prompts_received, 1)
            
            # 2. Add an empty message p4 and a real message p5 (mentions bot)
            await posts_queue.put([
                {"id": "p4", "user_id": "user_1", "channel_id": "chan_1", "root_id": "p1", "message": "   ", "create_at": 1100},
                {"id": "p5", "user_id": "user_1", "channel_id": "chan_1", "root_id": "p1", "message": "@goose-bot next", "create_at": 1200}
            ])
            await wait_for_prompts(prompts_received, 2)
    ...
    assert "joined an existing thread with 1 earlier messages" in prompts_received[0]
    # The second prompt should have the clearing signal because the first one had a hint
    assert "SYSTEM: You are now caught up with the thread." in prompts_received[1]